...
class TherapistPatientsByCategoryView(APIView):
    """
    GET /api/therapist/patients/vip-returning/
    Returns the logged-in therapist's patients filtered to VIP and Returning categories.

    Optional filter:
    ?category=VIP        → only VIP patients
    ?category=Returning  → only Returning patients
    (no ?category param) → both VIP and Returning combined

    Optional search:
    ?search=name|phone|email|patient_id
    """
    permission_classes = [IsAuthenticated]

    ALLOWED_CATEGORIES = ['VIP', 'Returning']
    ...
    def get(self, request):
        ...
        # Patients who have appointments with this therapist
        patient_ids = Appointment.objects.filter(
            staff=therapist
        ).values_list('patient_id', flat=True).distinct()

        qs = Patient.objects.filter(id__in=patient_ids).order_by('-created_at')

        # Restrict to VIP / Returning only
        category = request.query_params.get('category', '').strip()
        if category:
            if category not in self.ALLOWED_CATEGORIES:
                return Response(
                    {'error': f"Invalid category. Allowed values: {', '.join(self.ALLOWED_CATEGORIES)}"},
                    status=400
                )
            qs = qs.filter(category=category)
        else:
            qs = qs.filter(category__in=self.ALLOWED_CATEGORIES)
        ...
        # Build counts per category for the summary
        vip_count       = qs.filter(category='VIP').count()
        returning_count = qs.filter(category='Returning').count()

        return Response({
            'summary': {
                'total':     vip_count + returning_count,
                'vip':       vip_count,
                'returning': returning_count,
            },
            'patients': PatientSerializer(qs, many=True).data,
        })
